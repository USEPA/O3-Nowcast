...
Example.r:  Top level script that performs some overhead and calls the other pieces to run on example dataset
...
RunNowcast_local_v2.r: Contains the run.nowcast() function to apply the Nowcast calculation to a data stream, exemplified in Example.r. 

ExampleData.csv:  Example data upon which the Nowcast is calculated by Example.r
